...
int main()
{
	...
	Shader shaderDepthMap("shader/chapter/4-advanced-lighting/point-shadows/depth-map.vert", "shader/chapter/4-advanced-lighting/point-shadows/depth-map.frag", "shader/chapter/4-advanced-lighting/point-shadows/depth-map.geom");
	...
	Shader shader("shader/chapter/4-advanced-lighting/point-shadows/lighting.vert", "shader/chapter/4-advanced-lighting/point-shadows/lighting.frag");
	shader.use();
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		shaderDepthMap.use();
		shaderDepthMap.setFloat("far_plane", far_plane);
		shaderDepthMap.setFloat3("lightPos", lightPos.x, lightPos.y, lightPos.z);
		for (unsigned int i = 0; i < 6; ++i)
			shaderDepthMap.setMat4f("shadowMatrices[" + std::to_string(i) + "]", glm::value_ptr(shadowTransforms[i]));

		// room cube
		glm::mat4 model = glm::scale(identity, glm::vec3(5.0f));
		shaderDepthMap.setMat4f("model", glm::value_ptr(model));
		glDisable(GL_CULL_FACE); // note that we disable culling here since we render 'inside' the cube instead of the usual 'outside' which throws off the normal culling methods.
		shaderDepthMap.setInt("reverse_normals", 1); // A small little hack to invert normals when drawing cube from the inside so lighting still works.
		glBindVertexArray(VAOCubeId);
		glDrawArrays(GL_TRIANGLES, 0, 36);
		shaderDepthMap.setInt("reverse_normals", 0); // and of course disable it
		glEnable(GL_CULL_FACE);

		// cubes
		glm::mat4 cubeModelMatrix = glm::translate(identity, glm::vec3(4.0f, -3.5f, 0.0));
		cubeModelMatrix = glm::scale(cubeModelMatrix, glm::vec3(0.5f));
		shaderDepthMap.setMat4f("model", glm::value_ptr(cubeModelMatrix));
		glDrawArrays(GL_TRIANGLES, 0, 36);
				
		cubeModelMatrix = glm::translate(identity, glm::vec3(2.0f, 3.0f, 1.0));
		cubeModelMatrix = glm::scale(cubeModelMatrix, glm::vec3(0.75f));
		shaderDepthMap.setMat4f("model", glm::value_ptr(cubeModelMatrix));
		glDrawArrays(GL_TRIANGLES, 0, 36);

		cubeModelMatrix = glm::translate(identity, glm::vec3(-3.0f, -1.0f, 0.0));
		cubeModelMatrix = glm::scale(cubeModelMatrix, glm::vec3(0.5f));
		shaderDepthMap.setMat4f("model", glm::value_ptr(cubeModelMatrix));
		glDrawArrays(GL_TRIANGLES, 0, 36);

		cubeModelMatrix = glm::translate(identity, glm::vec3(-1.5f, 1.0f, 1.5));
		cubeModelMatrix = glm::scale(cubeModelMatrix, glm::vec3(0.5f));
		shaderDepthMap.setMat4f("model", glm::value_ptr(cubeModelMatrix));
		glDrawArrays(GL_TRIANGLES, 0, 36);

		cubeModelMatrix = glm::translate(identity, glm::vec3(-1.5f, 2.0f, -3.0));
		...
		// room cube
		model = glm::scale(identity, glm::vec3(5.0f));
		...
		// cubes
		cubeModelMatrix = glm::translate(identity, glm::vec3(4.0f, -3.5f, 0.0));
		cubeModelMatrix = glm::scale(cubeModelMatrix, glm::vec3(0.5f));
