...
int main()
{
	...
	Shader shaderCube("shader/chapter/1-lighting/light-casters/vertex-color-directional.vert", "shader/chapter/1-lighting/light-casters/fragment-color-multiple.frag");

	shaderCube.use();
	shaderCube.setInt("material.shininess", 32);

	shaderCube.setFloat3("directionalLight.specular", 0.5f, 0.5f, 0.5f);
	shaderCube.setFloat3("directionalLight.ambient", 0.05f, 0.05f, 0.05f);
	shaderCube.setFloat3("directionalLight.diffuse", 0.2f, 0.2f, 0.2f);
	shaderCube.setFloat3("directionalLight.direction", -0.2f, -1.0f, -0.3f);
	...
	for (int i = 0; i < 4; i++)
	{
		...
		shaderCube.setFloat3("pointLights[" + index + "].position", pointLightPositions[i].x, pointLightPositions[i].y, pointLightPositions[i].z);
		shaderCube.setFloat3("pointLights[" + index + "].ambient", 0.05f, 0.05f, 0.05f);
		shaderCube.setFloat3("pointLights[" + index + "].diffuse", 0.8f, 0.8f, 0.8f);
		shaderCube.setFloat3("pointLights[" + index + "].specular", 1.0f, 1.0f, 1.0f);
		shaderCube.setFloat("pointLights[" + index + "].constant", 1.0f);
		shaderCube.setFloat("pointLights[" + index + "].linear", 0.14f);
		shaderCube.setFloat("pointLights[" + index + "].quadratic", 0.07f);
	}
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		/*shaderCube.setFloat3("light.position", camera.Position.x, camera.Position.y, camera.Position.z);
		shaderCube.setFloat3("light.direction", camera.Front.x, camera.Front.y, camera.Front.z);*/
		...
		for (unsigned int i = 0; i < 10; i++)
		{
			...
			modelTransformation = glm::rotate(modelTransformation, glm::radians(angle), glm::vec3(1.0f, 0.3f, 0.5f));
			shaderCube.setMat4f("model", glm::value_ptr(modelTransformation));
		...
		for (int i = 0; i < 4; i++)
		{
			glm::mat4 lightTransform = glm::translate(glm::mat4(1.0f), glm::vec3(pointLightPositions[i].x, pointLightPositions[i].y, pointLightPositions[i].z));
	...
	glDeleteTextures(1, &textureSpecularId);
